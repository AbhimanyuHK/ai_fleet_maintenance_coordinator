...
def load_group(folder, files):
    return {name: load_csv(folder, filename) for name, filename in files.items() if (folder / filename).exists()}

def show_table(df, key, height=400):
    if df.empty:
        st.info("No data available.")
        return
    q = st.text_input("Search", key=key)
    view = df
    if q.strip():
...
def count_value(df, col, value):
    return int(df[col].astype(str).str.upper().eq(value.upper()).sum()) if col in df else 0

def count_true(df, col):
    return int(df[col].astype(str).str.lower().eq("true").sum()) if col in df else 0
...
history = p1.get("Maintenance History", pd.DataFrame())
...
locations = p1.get("Service Locations", pd.DataFrame())
...
review_queue = build_review_queue(P2)
reconciliation = reconcile_estimates_invoices(P2)
...
if section == "📊 Data Foundation":
    ...
    with tabs[0]:
        c=st.columns(6); c[0].metric("Equipment",len(fleet)); c[1].metric("PM Records",len(pm)); c[2].metric("Fault Events",len(faults)); c[3].metric("Open Work Orders",count_value(work_orders,"status","OPEN")); c[4].metric("Driver Requests",len(requests)); c[5].metric("Review Items",len(review_queue))
        a,b=st.columns(2)
        with a:
    ...
    with tabs[1]:
        st.subheader("PM Management"); c=st.columns(3); c[0].metric("Scheduled PM",len(pm)); c[1].metric("Due / Upcoming",sum(count_value(pm,"status",x) for x in ["DUE","UPCOMING","DUE_SOON"])); c[2].metric("Overdue",count_value(pm,"status","OVERDUE")); show_table(pm,"pm_search")
    with tabs[2]:
        st.subheader("Faults & Alerts"); c=st.columns(4); c[0].metric("Total Faults",len(faults)); c[1].metric("Critical",count_value(faults,"severity","CRITICAL")); c[2].metric("High",count_value(faults,"severity","HIGH")); c[3].metric("Safety Related",count_true(faults,"safety_related")); show_table(faults,"fault_search")
    with tabs[3]:
        st.subheader("Work Orders & Repairs"); c=st.columns(4); c[0].metric("Work Orders",len(work_orders)); c[1].metric("Open",count_value(work_orders,"status","OPEN")); c[2].metric("Repair Notes",len(repair_notes)); c[3].metric("Unavailable Fleet",len(availability)-count_value(availability,"status","AVAILABLE")); show_table(work_orders,"wo_search"); st.subheader("Repair Notes"); show_table(repair_notes,"repair_notes_search",300)
    with tabs[4]:
    ...
    with tabs[5]:
        st.subheader("Vendor Management"); c=st.columns(3); c[0].metric("Vendors",len(vendors)); c[1].metric("Service Locations",len(locations)); c[2].metric("Vendor Estimates",len(estimates)); show_table(vendors,"vendor_search"); st.subheader("Service Locations"); show_table(locations,"location_search",300)
    with tabs[6]:
    ...
    with tabs[8]:
        st.subheader("Data Quality"); rows=[{"Dataset":n,"Rows":len(df),"Columns":len(df.columns),"Missing Cells":int(df.isna().sum().sum())} for n,df in {**p1,**p2}.items()]; st.dataframe(pd.DataFrame(rows),use_container_width=True,hide_index=True); st.subheader("Human Review Queue"); show_table(review_queue,"review_search",350)

elif section == "🤖 AI Intelligence":
    st.header("🤖 AI Intelligence")
    tabs=st.tabs(["Maintenance Knowledge Assistant","Maintenance Recommendations","Communication Assistant","Repair Recommendations","Maintenance Scheduling","Predictive Risk"])
    with tabs[0]:
            ...
            if not question.strip(): st.warning("Enter a question first.")
                ...
                if not results: st.error("No approved knowledge evidence was retrieved.")
                    ...
                    for i,r in enumerate(results,1):
                        chunk=r["chunk"]; label=f"[{i}] {chunk.document_id} — {chunk.title} — authority={chunk.authority} — score={r['score']}"; evidence.append(label+"\n"+chunk.text); st.markdown(f"**{label}**"); st.write(chunk.text)
    ...
    with tabs[1]:
        ...
        if fleet.empty: st.info("No equipment data available.")
            ...
            eq_col="equipment_id" if "equipment_id" in fleet.columns else fleet.columns[0]; eq=st.selectbox("Equipment",fleet[eq_col].astype(str).tolist(),key="maintenance_equipment")
            f=faults[faults[eq_col].astype(str).eq(eq)] if eq_col in faults.columns else faults; p=pm[pm[eq_col].astype(str).eq(eq)] if eq_col in pm.columns else pm; h=history[history[eq_col].astype(str).eq(eq)] if eq_col in history.columns else history; d=requests[requests[eq_col].astype(str).eq(eq)] if eq_col in requests.columns else requests
            if st.button("Generate Maintenance Recommendation",type="primary",key="maintenance_recommendation"):
                result=recommend(eq," ".join(f.astype(str).fillna("").agg(" ".join,axis=1))," ".join(d.astype(str).fillna("").agg(" ".join,axis=1))," ".join(p.astype(str).fillna("").agg(" ".join,axis=1)+h.astype(str).fillna("").agg(" ".join,axis=1)),load_chunks(KNOWLEDGE)); c=st.columns(4); c[0].metric("Priority",result.priority); c[1].metric("Evidence",len(result.evidence)); c[2].metric("Approval","Required"); c[3].metric("Equipment",result.equipment_id); st.subheader("Recommendation"); st.write(result.recommendation); st.subheader("Reasons"); [st.write(f"• {x}") for x in result.reasons]; st.subheader("Evidence Sources"); [st.write(f"• {x}") for x in result.evidence]; st.warning("No autonomous action is taken.")
    with tabs[2]:
        st.subheader("Communication Assistant"); st.caption("Classify driver communication and prepare a response draft. Nothing is sent automatically.")
        options=["Manual message"]+(["Driver request"] if not requests.empty else [])+(["Email/message"] if not messages.empty else [])+(["Voicemail"] if not voicemails.empty else []); source=st.selectbox("Input source",options); eq=None; text=""
    ...
    with tabs[3]:
        render_repair_recommendations(faults,history,estimates,repair_notes,KNOWLEDGE)
    for tab,title,desc in [(tabs[4],"Maintenance Scheduling","Recommend low-disruption service timing from PM due dates, availability, vendor preferences and operational constraints."),(tabs[5],"Predictive Risk","Estimate failure risk from maintenance history, fault patterns, telemetry and operational signals with explainable factors.")]:
        with tab:
            st.subheader(title); st.write(desc); c=st.columns(3); c[0].metric("Status","Foundation Ready"); c[1].metric("Model","Qwen 2.5 3B"); c[2].metric("Human Approval","Required"); st.info("This capability will consume validated operational data + RAG evidence. No autonomous operational decision is enabled.")
else:
